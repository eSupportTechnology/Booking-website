...
<body class="bg-gray-100 text-gray-800">
    ...
    <!-- Start Form -->
    <div class="max-w-6xl p-4 ml-14 bg-gray-100" x-data="propertySubtypeComponent()" x-init="init()">
        ...
        <form class="p-6 rounded-lg space-y-6" @submit.prevent>
                ...
                <div class="bg-white max-w-2xl w-full p-6 rounded-lg shadow mx-auto" x-data="{ selected: '' }">
                    ...
                    <label
                        ...
                        <div class="flex items-center space-x-4">
                            <img src="{{ asset('images/' . ($subcategory->icon ?? 'default.png')) }}" alt="{{ $subcategory->name }}" class="w-8 h-8" />
                            <div>
                                <span class="text-base font-bold text-gray-800">{{ $subcategory->name }}</span>
                                <p class="text-xs text-gray-800">
                                    {{ $subcategory->description }}
                                </p>
                            </div>
                        </div>
                        <input type="radio" name="apartment_type" value="{{ $subcategory->id }}" x-model="selected" class="hidden" />
                    </label>
                    @endforeach
            ...
            <div id="selection-container" x-show="step === 2" x-cloak class="container mx-auto px-4 py-8 max-w-6xl">
                ...
                <div x-show="step === 2" class="bg-white p-6 rounded-lg shadow">
...
<script>
    const propertySubtypeUrl = "{{ url('/partner/property_subtype') }}";

    function propertySubtypeComponent() {
        return {
            selected: '',
            step: 1,
            subStep: 1,
            ...
            async fetchSubtypes() {
                ...
                try {
                    const response = await fetch(`${propertySubtypeUrl}/${this.selected}`);
            ...
            init() {
                // Optionally fetch defaults or preload
            }
        }
    }
</script>
