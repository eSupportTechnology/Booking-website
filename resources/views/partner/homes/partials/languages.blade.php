<div class="p-4 sm:p-6 md:p-8">
    <div class="mb-6 sm:mb-8">
        <h3 class="text-xl sm:text-2xl font-bold text-gray-900 mb-2">Languages Spoken</h3>
        <p class="text-gray-600 text-sm sm:text-base">Let guests know which languages you can communicate in</p>
    </div>

    <form id="languages-form" class="space-y-6 sm:space-y-8" action="{{ route('partner.homes.update.languages', $property) }}" method="POST">
        @csrf
        ...
        <div class="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-4 sm:p-6 border border-purple-100">
            <div class="flex items-center mb-4 sm:mb-6 flex-wrap">
                <div class="bg-purple-100 p-3 rounded-xl mr-4 mb-2 sm:mb-0">
                    <i class="fas fa-globe text-purple-600 text-lg sm:text-xl"></i>
                </div>
                <div>
                    <h4 class="text-base sm:text-lg font-bold text-gray-900">Popular Languages</h4>
                ...
                @foreach($popularLanguages as $language)
                    <label class="flex items-center p-3 sm:p-4 bg-white rounded-xl border-2 {{ in_array($language['id'], $selectedLanguages) ? 'border-purple-500 bg-purple-50' : 'border-gray-200' }} hover:border-purple-300 cursor-pointer transition-all duration-200 group transform hover:scale-105 language-checkbox peer-checked:border-purple-500 peer-checked:bg-purple-50">
                        ...
                        <div class="flex items-center">
                            <span class="text-base sm:text-lg mr-2">{{ $language['flag'] }}</span>
                            <span class="font-medium text-gray-700 group-hover:text-purple-700 text-sm sm:text-base">{{ $language['name'] }}</span>
                        </div>
                    </label>
                @endforeach
            </div>
        ...
        <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-4 sm:p-6 border border-blue-100">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6">
                <div class="flex items-center mb-3 sm:mb-0">
                    ...
                    <div>
                        <h4 class="text-base sm:text-lg font-bold text-gray-900">Additional Languages</h4>
                        <p class="text-gray-600 text-sm">Add any other languages you speak</p>
                    </div>
                </div>
                <button type="button" class="bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 text-white font-medium py-2 px-5 sm:px-6 rounded-xl transition-all duration-200 transform hover:scale-105" id="add-language">
        ...
        {{-- Section left intact as requested --}}

        <div class="flex flex-col sm:flex-row sm:justify-end sm:items-center pt-6 space-y-3 sm:space-y-0 sm:space-x-4">
            <button type="submit" class="bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white font-bold py-3 px-6 sm:px-8 rounded-xl shadow-lg transform transition-all duration-200 hover:scale-105 w-full sm:w-auto">
