  ...
  <div class="max-w-4xl mx-auto bg-white p-6 rounded shadow space-y-8">
    ...
    <div class="space-y-1">
      <h2 class="text-xl font-semibold">
        ...
        <span class="inline-block bg-gray-200 text-sm px-2 py-0.5 rounded-full">{{ $property->id ?? '1' }}</span>
      </h2>
      <p class="text-sm text-gray-600">
        {{ $propertyData['address'] ?? 'Address not set' }}
      </p>
    </div>
    ...
    <div id="apartmentCardsContainer" class="space-y-8 mt-6">
      ...
      <div class="border rounded overflow-hidden apartment-card" data-apartment='{
        "sleepingArrangements": "{{ isset($roomDisplayData['bedroom']) ? $roomDisplayData['bedroom']['bed_summary'] : 'No beds added' }}",
        "amenities": "WiFi, Air conditioning",
        "photosCount": {{ count($propertyData['photos'] ?? []) }},
        "basePrice": 123,
            ...
            <div class="relative w-full h-48 sm:h-full overflow-hidden rounded-t sm:rounded-l sm:rounded-t-none">
              <div class="relative w-full h-48 bg-red-200">
                @if(!empty($propertyData['photos']))
                    <img src="{{ asset('storage/' . $propertyData['photos'][0]) }}" alt="Apartment" class="w-full h-full object-cover" />
                @else
          ...
          <div class="sm:col-span-6 p-4 space-y-2">
            ...
            <div class="flex flex-col sm:flex-row text-sm divide-y sm:divide-y-0 sm:divide-x divide-gray-300">
              <div class="sm:pr-6 pb-2 sm:pb-0">
                ...
                <p class="font-medium">{{ isset($roomDisplayData['bedroom']) ? $roomDisplayData['bedroom']['total_beds'] : '0' }}</p>
              </div>
              <div class="sm:px-6 pb-2 sm:pb-0">
                <p class="text-gray-500">Guests</p>
                <p class="font-medium">{{ $propertyData['guests_capacity'] ?? '0' }}</p>
              </div>
              <div class="sm:pl-6">
