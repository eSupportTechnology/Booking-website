...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-lg p-5 border border-gray-200 shadow-sm">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            <div>
                <h1 class="text-xl font-semibold text-gray-800">Homes</h1>
                <p class="text-gray-500 text-sm">Manage your home listings</p>
            </div>
    ...
    <div class="bg-white rounded-lg p-4 border border-gray-200 shadow-sm">
        <form method="GET" class="flex flex-col sm:flex-row gap-3">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search homes..."
            ...
            <a href="{{ route('partner.properties.homes') }}" class="bg-gray-100 text-gray-700 px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-200">Clear</a>
            @endif
        </form>
    </div>
    ...
    <div class="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div class="px-4 py-3 bg-gray-50 border-t border-gray-200">
            <div class="flex flex-col sm:flex-row justify-between items-center gap-3">
                <p class="text-sm text-gray-500">
                    Showing {{ $properties->firstItem() ?? 0 }} to {{ $properties->lastItem() ?? 0 }} of {{ $properties->total() }} home(s)
                </p>
        ...
        <div class="text-center py-12">
            ...
            <p class="text-gray-500 text-sm mb-3">No homes found</p>
            <a href="{{ route('partner.list-your-property') }}" class="text-[#1F8FB2] text-sm hover:underline">Add your first home</a>
        </div>
        @endif
    </div>
</div>
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetch('/partner/properties/bulk-update', {
                ...
                body: JSON.stringify({ property_ids: selectedIds, action: action })
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    Swal.fire({ title: 'Success!', text: data.message || successMessage, icon: 'success', confirmButtonColor: '#1F8FB2' }).then(() => location.reload());
                } else {
                    Swal.fire({ title: 'Error!', text: data.message || 'Operation failed.', icon: 'error', confirmButtonColor: '#1F8FB2' });
                }
            })
            .catch(error => {
                Swal.fire({ title: 'Error!', text: 'Something went wrong. Please try again.', icon: 'error', confirmButtonColor: '#1F8FB2' });
            });
        }
    });
}

function bulkDelete() { bulkAction('delete', 'Delete Selected Properties?', 'Are you sure you want to delete {count} properties? This action cannot be undone.', 'Properties deleted successfully.'); }
function bulkActivate() { bulkAction('activate', 'Activate Selected Properties?', 'Are you sure you want to activate {count} properties?', 'Properties activated successfully.'); }
function bulkDeactivate() { bulkAction('deactivate', 'Deactivate Selected Properties?', 'Are you sure you want to deactivate {count} properties?', 'Properties deactivated successfully.'); }
...
function deleteProperty(propertyId) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            .then(data => {
                if (data.success) {
                    Swal.fire({ title: 'Deleted!', text: 'Property has been deleted successfully.', icon: 'success', confirmButtonColor: '#1F8FB2' }).then(() => location.reload());
                } else {
                    Swal.fire({ title: 'Error!', text: data.message || 'Failed to delete property.', icon: 'error', confirmButtonColor: '#1F8FB2' });
                }
            })
            .catch(error => { Swal.fire({ title: 'Error!', text: 'Something went wrong. Please try again.', icon: 'error', confirmButtonColor: '#1F8FB2' }); });
        }
    });
}
</script>
