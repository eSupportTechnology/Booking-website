...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        <h1 class="text-3xl font-semibold text-gray-800">Hotel Listings</h1>
    ...
    <div class="bg-white p-4 rounded-lg shadow">
        <form method="GET" class="flex space-x-4">
            <input type="text" name="search" value="{{ $search }}" placeholder="Search hotels by name or location..." class="flex-1 border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
            <button type="submit" class="bg-blue-500 text-white px-6 py-2 rounded-lg hover:bg-blue-600">Search</button>
            @if($search)
                <a href="{{ route('partner.properties.hotels') }}" class="bg-gray-500 text-white px-6 py-2 rounded-lg hover:bg-gray-600">Clear</a>
            @endif
        </form>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-4 gap-6">
        <div class="bg-white p-6 rounded-lg shadow border-l-4" style="border-color: #1F8FB2;">
            <h2 class="text-sm text-gray-500">Total Hotels</h2>
            <p class="text-2xl font-bold text-gray-800">{{ $counts->hotels }}</p>
        </div>
    ...
    <div class="bg-white p-6 rounded-lg shadow">
        <h2 class="text-lg font-semibold text-gray-700 mb-4">Hotel Properties</h2>
