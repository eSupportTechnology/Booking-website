...
<section class="bg-gray-50 py-10">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 class="text-2xl font-extrabold text-gray-900">Available Taxis</h2>
        <p class="mt-2 mb-8 text-gray-600">Browse available active taxis with or without a driver</p>

        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            @forelse($activeTaxis as $taxi)
                <div class="bg-white shadow-md rounded-lg overflow-hidden hover:shadow-xl transition-shadow">
                    ...
                    <div class="p-4">
                        <h3 class="text-lg font-bold">{{ $taxi->type->name ?? 'Taxi' }}</h3>
                        <p class="text-sm text-gray-500">
                            Renter: <span class="font-medium text-gray-700">{{ $taxi->renter->name ?? 'Unknown' }}</span>
                        </p>

                        <p class="text-gray-600 mt-1">
                            {{ $taxi->passenger_capacity }} seats • {{ $taxi->luggage_capacity }} luggage • {{ ucfirst($taxi->color) }}
                        </p>

                        <div class="mt-2 flex justify-between items-center">
                            <span class="text-blue-600 font-semibold">
                                ${{ $taxi->fare->price ?? '0' }}/day
                            </span>
            ...
            @empty
                <div class="col-span-full text-center py-8">
                    <p class="text-gray-500 text-lg">No active taxis available at the moment.</p>
                </div>
            @endforelse
        </div>

        @if($activeTaxis->hasPages())
